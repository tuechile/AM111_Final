...
and automatic point selection via learnable importance weights (Method B)
plus a search for the smallest number of points K that meets a target MSE.
...
4. Phase 1 (importance learning):
    ...
    - This yields an importance weight for every point.
5. Use the learned weights to define an ordering of points (most to least important).
6. Phase 2 (K-search):
    - For different candidate K (number of points), train a small MLP on the top-K points.
    - Evaluate MSE on the full curve.
    - Use a coarse binary-search-like procedure to find the smallest K such that MSE ≤ TARGET_MSE.
7. Phase 3 (final training):
    - Retrain a final MLP on the chosen K points with full epochs.
8. Export the final model (weights, SVG, PNG) and diagnostics into a timestamped folder.
"""
...
# Importance learning (Phase 1)
IMPORTANCE_EPOCHS_MULTIPLIER = 1.0   # importance_epochs = multiplier * epochs
LAMBDA_SPARSITY = 1e-3               # weight sparsity penalty strength
SIGMOID_EPS = 1e-8                   # numerical epsilon

# K-search (Phase 2)
SEARCH_EPOCHS_MULTIPLIER = 0.3       # num_epochs_for_search = multiplier * epochs
MAX_SEARCH_STEPS = 8                 # max evaluations of K during search
K_TOL = 20                           # stop search when K_high - K_low <= K_TOL

# Network architecture (used in all phases)
HIDDEN_DIM = 64
NUM_HIDDEN_LAYERS = 2
...
# -----------------------------
# 5. Method B: learnable weights (Phase 1)
# -----------------------------
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 200 == 0:
            print(
                ...
                f"loss={loss.item():.6e}, mean(w)={mean_weights_history[-1]:.4f}"
            )
...
# -----------------------------
# 6. K-search utilities (Phase 2)
# -----------------------------

def get_top_k_indices_by_weight(weights, K):
    """
    Return indices of the top-K weights (descending).
    """
    N = len(weights)
    K = max(1, min(K, N))
    sorted_idx = np.argsort(-weights)  # descending
    keep_idx = np.sort(sorted_idx[:K])
    return keep_idx


def train_and_eval_for_K(s_full, coords_full, weights, K, epochs_for_search, lr):
    """
    For a candidate K:
        - select top-K points by weight
        - train a small MLP on these K points
        - evaluate MSE on the full curve
    Returns (mse_full, K).
    """
    keep_idx = get_top_k_indices_by_weight(weights, K)
    s_sel = s_full[keep_idx]
    coords_sel = coords_full[keep_idx]

    print(f"  [Search] Training on K={K} points (subset)...")
    model_K, _ = train_curve_mlp(
        s_sel,
        coords_sel,
        num_epochs=epochs_for_search,
        lr=lr,
    ...
    mse_full = compute_mse(model_K, s_full, coords_full)
    print(f"  [Search] K={K}, full-curve MSE={mse_full:.6e}")
    return mse_full
...
def search_best_K(
    ...
    """
    Use a coarse binary-search-like procedure on K (number of points) to find
    the smallest K such that MSE(K) <= target_mse.

    If no K satisfies the target, returns K with the smallest MSE.
    """
    ...
    if K_low >= K_high:
        print("  [Search] MIN_POINTS >= N; using all points.")
        mse_full = train_and_eval_for_K(s_full, coords_full, weights, N, epochs_for_search, lr)
        return N, mse_full

    print(f"\n=== Phase 2: K-search in [{K_low}, {K_high}] for target MSE={target_mse:.2e} ===")

    best_K = None
    best_K_mse = float("inf")

    # Evaluate at K_high to see if target is even achievable
    mse_high = train_and_eval_for_K(s_full, coords_full, weights, K_high, epochs_for_search, lr)
    best_K = K_high
    best_K_mse = mse_high

    if mse_high > target_mse:
        print("  [Search] Even K=N does not meet target MSE. Using K=N.")
        return K_high, mse_high

    # Evaluate at K_low
    mse_low = train_and_eval_for_K(s_full, coords_full, weights, K_low, epochs_for_search, lr)
    if mse_low <= target_mse and K_low < best_K:
        best_K = K_low
        best_K_mse = mse_low

    # Binary-search-like loop
    steps = 0
    while steps < max_steps and (K_high - K_low) > k_tol:
        steps += 1
        K_mid = (K_low + K_high) // 2
        print(f"\n  [Search step {steps}] Trying K_mid={K_mid}")
        mse_mid = train_and_eval_for_K(s_full, coords_full, weights, K_mid, epochs_for_search, lr)

        if mse_mid <= target_mse:
            # Feasible: try to reduce K further
            if mse_mid < best_K_mse or (mse_mid <= target_mse and K_mid < best_K):
                best_K = K_mid
                best_K_mse = mse_mid
            K_high = K_mid
        else:
            # Not feasible: need more points
            K_low = K_mid

    print(f"\n  [Search] Best K found: {best_K} with full-curve MSE={best_K_mse:.6e}")
    return best_K, best_K_mse
...
def main():
    ...
    # 5) Phase 2: search best K using learned weights
    ...
    # 6) Phase 3: final training on best_K points with full epochs
    print(f"\n=== Phase 3: Final training on K={best_K} points ({epochs} epochs) ===")
    ...
    np.save(out_dir / "selected_indices_bestK.npy", final_keep_idx)
